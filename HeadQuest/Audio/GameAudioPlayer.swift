...
class GameAudioPlayer
{
	...
	func play(fileName: String, loop: Bool, onPlayed: @escaping () -> Void) throws
	{
		...
		playerNode.scheduleBuffer(audioBuffer, at: nil, options: options, completionCallbackType: AVAudioPlayerNodeCompletionCallbackType.dataPlayedBack)
		{ _ in
			onPlayed()
		}
...
class GameAudioPlayerWithMultipleAudios
{
	...
	{
		guard let playerNode = soundsPlayerNodes[fileName]
		else
		{
	...
	func stop()
	{
		...
		soundsPlayerNodes.values.forEach
		{ node in
			node?.stop()
		}
...
enum GameAudioUtils
{
	...
	{
		guard let audioBuffer = AVAudioPCMBuffer(pcmFormat: audioFile.processingFormat, frameCapacity: AVAudioFrameCount(audioFile.length))
		else
		{
