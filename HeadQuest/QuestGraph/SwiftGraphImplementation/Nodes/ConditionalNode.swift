...
public class ConditionalNode: QuestGraphNodeSG
{
	...
	init(name: String, gameStatePropertyToCheck: GameStateConstants, expectedValue: String)
	{
		...
		super.init(name: name, description: nil, isEnd: false, backgroundMusicFile: nil, preVoiceSound: nil, postVoiceSound: nil, isSkipable: true)
	}

	// TODO: extend to support not only equalence
	public func evaluateCondition() -> Bool
	{
		GameStateWithDictionary.shared.readState(for: gameStatePropertyToCheck.rawValue) == expectedValue
	}
...
public enum ConditionalEdgeTypes
{
